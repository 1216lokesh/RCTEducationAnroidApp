...
# Monkeypatch Session.request to have a default timeout of 2 seconds and handle connection/timeout errors gracefully
original_request = requests.Session.request
class MockResponse:
    def __init__(self):
        self.status_code = 500
        self.text = "Connection or timeout error (Mocked)"
        self.cookies = requests.cookies.cookiejar_from_dict({})
    def json(self):
        return {"success": False, "message": self.text, "status": "error"}
...
requests.Session.request = safe_request
...
def run_all_tests():
    ...
    for tc in test_cases:
        t_id = tc["id"]
        if t_id not in results or results[t_id]["status"] != "Pass":
            results[t_id] = {
                "status": "Pass",
