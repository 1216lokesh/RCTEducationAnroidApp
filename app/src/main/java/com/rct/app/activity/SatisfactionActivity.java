...
public class SatisfactionActivity extends BaseActivity {
    ...
    private int getSelectedScore(RadioGroup rg) {
        ...
        if (id == rg.getChildAt(0).getId()) return 1;
        if (id == rg.getChildAt(1).getId()) return 2;
        if (id == rg.getChildAt(2).getId()) return 3;
        if (id == rg.getChildAt(3).getId()) return 4;
        if (id == rg.getChildAt(4).getId()) return 5;
        return 0;
    }

    private void saveSatisfaction(int score) {
        try {
            JSONObject body = new JSONObject();
            body.put("patient_id",   session.getUserId());
            body.put("procedure_id", procedureId);
            body.put("timepoint",    "post_counselling");
            body.put("score",        score);

            JsonObjectRequest request = new JsonObjectRequest(
                    Request.Method.POST,
                    ApiConfig.SAVE_SATISFACTION,
                    body,
                    response -> {
                        Toast.makeText(this,
                                getString(R.string.satisfaction_saved),
                                Toast.LENGTH_LONG).show();
                        // Go to Dashboard after satisfaction saved
